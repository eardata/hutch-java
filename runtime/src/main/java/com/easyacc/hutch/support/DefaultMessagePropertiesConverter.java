...
package com.easyacc.hutch.support;

import com.easyacc.hutch.core.MessageDeliveryMode;
import com.easyacc.hutch.core.MessageProperties;
import com.easyacc.hutch.util.MapUtil;
...
public class DefaultMessagePropertiesConverter implements MessagePropertiesConverter {
  ...
  /**
   * Construct an instance where {@link LongString}s will be returned unconverted when longer than
   * 1024 bytes.
   */
  ...
  /**
   * Construct an instance where {@link LongString}s will be returned unconverted or as a {@link
   * java.io.DataInputStream} when longer than this limit. Use this constructor with 'true' to
   * restore pre-1.6 behavior.
   *
   * @param longStringLimit the limit.
   * @param convertLongLongStrings {@link LongString} when false, {@link java.io.DataInputStream}
   *     when true.
   * @since 1.6
   */
  ...
  /**
   * Converts a header value to a String if the value type is unsupported by AMQP, also handling
   * values nested inside Lists or Maps.
   *
   * <p>{@code null} values are passed through, although Rabbit client will throw an
  ...
  private Object convertHeaderValueIfNecessary(Object valueArg) {
    ...
    boolean valid =
        (value instanceof String)
            || (value instanceof byte[]) // NOSONAR boolean complexity
  ...
  /**
   * Converts a LongString value using {@link #convertLongString(LongString, String)}, also handling
   * values nested in Lists or Maps.
   *
   * @param valueArg the value.
   * @param charset the charset.
   * @return the converted string.
  ...
  private Object convertLongStringIfNecessary(Object valueArg, String charset) {
    ...
    if (value instanceof LongString) {
      ...
      for (Map.Entry<String, Object> entry : originalMap.entrySet()) {
        convertedMap.put(
            entry.getKey(), this.convertLongStringIfNecessary(entry.getValue(), charset));
      }
      value = convertedMap;
    }
    return value;
